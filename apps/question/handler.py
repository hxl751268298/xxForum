...
from apps.utils.decorator import authenticated_async
...
class AnswerReplyHandler(RedisHandler):
    ...
    async def get(self, answer_id, *args, **kwargs):
        re_data = []
        answer_replys = await self.application.objects.execute(Answer.extend().where(Answer.parent_answer_id==int(answer_id)))
